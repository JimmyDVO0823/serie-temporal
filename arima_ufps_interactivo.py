"""
ARIMA interactivo y universal
- Lee formatos horizontales y verticales
- Permite elegir archivo, variable y tiempo desde consola
- Genera reporte, pronóstico y gráficas
...
def _parse_number_cell(v, default_mode="auto"):
    ...
    # Normalización en función del contexto de la serie.
    # default_mode puede ser:
    #   - "dot_thousands"
    #   - "dot_decimal"
    #   - "comma_decimal"
    #   - "auto"
    if default_mode == "dot_thousands":
        ...
        # mantener el punto decimal; si también hay coma, asumir coma decimal/miles
        if "," in s and "." in s:
            if s.rfind(",") > s.rfind("."):
                s = s.replace(".", "").replace(",", ".")
            else:
...
def normalize_numeric_series(s):
    ...
    comma_decimal = non_empty.str.match(r"^\d+,\d+$", na=False).sum()

    if has_comma and not has_dot:
        # Casi seguro decimal con coma
        mode = "comma_decimal"
    elif has_dot and not has_comma:
        # Si la mayoría coincide con miles tipo 1.326, usar miles; si no, decimal
...
def detect_format(path):
    df0, _ = read_csv_flexible(path)
    # Heurística: si la primera fila tiene muchos años y la primera columna parece etiquetas
    ...
    if year_like >= max(4, len(first_row) // 2):
        return "vertical"
    # Si las columnas ya vienen con nombre de tiempo o una serie típica
    try:
...
def choose_file_interactive():
    ...
    if len(files) == 1:
        ...
        return files[0]
    return ask_choice("Selecciona el archivo CSV para analizar:", files)
...
def load_horizontal(path):
    ...
    if not candidate_series:
        # fallback: cualquier columna no temporal
    ...
    # columnas extra de tiempo que no sean la serie
    ...
    if chosen_time == "Año + Trimestre":
        ...
        # índice de fila o cualquier columna residual
    ...
    # Para quarter, limpiar posibles NaN de año/quarter también
    if chosen_time == "Año + Trimestre":
...
def load_data_interactive(args):
    ...
    if args.variable:
        # Si el usuario pasó variable por argumento, intentamos usarla sin preguntar.
        pass
...
def make_future_labels(time_values, time_mode, steps):
    ...
    if time_mode == "quarter":
        # time_values son etiquetas tipo "2021 Q1"
...
def prepare_numeric_axis(time_values, time_mode):
    ...
    if time_mode == "quarter":
        # index numérico para modelar y etiqueta aparte
...
def run_arima_analysis(data, steps):
    ...
    if len(y) < 6:
        raise ValueError("La serie tiene muy pocos datos para hacer un ARIMA útil.")

    # Eje temporal para visualización
    x = prepare_numeric_axis(time_values, time_mode)

    # Para modelos, trabajamos solo con la serie y
    wt = np.diff(y)
    n = len(y)
    ...
    for name, order in models:
        try:
            ...
            # Filtro de coherencia del pronóstico para evitar explosiones numéricas
    ...
    # Si el filtro de pronóstico deja todo fuera, relajamos un poco la regla
    if not validos:
        validos = [r for r in results if r.get("res") is not None and r.get("ma_ok", True)]

    if not validos:
        raise RuntimeError("No se pudo estimar ningún modelo ARIMA válido.")
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        f.write(LINEA + "\n")
        f.write(f"  REPORTE ARIMA — {variable_name.upper()}\n")
        f.write(f"  Archivo: {file_path}\n")
        f.write("  Análisis interactivo\n")
        f.write(LINEA + "\n\n")
        ...
        f.write(LINEA + "\n\n")
        f.write("  Criterio: AICc mínimo entre modelos sin problemas de invertibilidad.\n\n")
    ...
    fig.suptitle(
        f"Análisis ARIMA — {variable_name}\n"
        f"Mejor modelo: {mejor['nombre']} | Fuente: {os.path.basename(file_path)}",
    ...
    ax1.plot(range(len(y)), y, 'o-', color=AZUL, lw=2, ms=6, label='Histórico', zorder=3)
    ax1.plot(range(d, len(fitted)), fitted[d:], 's--', color=NARANJ, lw=1.8,
             markersize=4, label='Valores ajustados', zorder=2)
    ax1.plot([len(y)-1] + list(range(len(y), len(y)+steps)),
             [y[-1]] + list(np.round(p_mu, 2)),
             's--', color=ROJO, lw=2, ms=7, label=f'Pronóstico {future_labels[0]}–{future_labels[-1]}', zorder=4)
    ax1.fill_between(range(len(y), len(y)+steps), p_ci[:, 0], p_ci[:, 1],
                     color=ROJO, alpha=0.12, label='IC 95%')
    ax1.set_title(f'Serie original, valores ajustados y pronóstico ({variable_name})', fontsize=11)
    ...
    ax2.bar(range(d, len(resid)), resid[d:], color=[ROJO if r < 0 else AZUL for r in resid[d:]],
            alpha=0.75, edgecolor='white', linewidth=0.5)
    ax2.axhline(0, color='black', lw=1)
    ax2.axhline(+2 * np.std(resid[d:]), color=GRIS, ls='--', lw=1, alpha=0.7)
    ax2.axhline(-2 * np.std(resid[d:]), color=GRIS, ls='--', lw=1, alpha=0.7)
    ax2.set_title('Residuales', fontsize=10)
    ax2.set_ylabel('Residual')
    ax2.set_xlabel('Índice')
    ax2.grid(alpha=0.2)

    ax3 = fig.add_subplot(gs[1, 1])
    acf_resid = acf(resid[d:], nlags=min(8, len(resid[d:]) - 1), fft=False)
    ...
    for bar, val in zip(bars, acf_resid[1:]):
        if abs(val) > lim95:
            bar.set_color(ROJO)
            bar.set_alpha(0.9)
    ax3.set_title('ACF de residuales', fontsize=10)
    ax3.set_xlabel('Lag')
    ...
    barras_acf(ax4l, acf_vals, 'FAC de Wt = Yt − Yt−1', AZUL)
    ...
    ax.plot([len(y)-1] + list(range(len(y), len(y)+steps)),
            [y[-1]] + list(np.round(p_mu, 2)),
            's--', lw=2.5, ms=8, label=f'Pronóstico {mejor["nombre"]}', zorder=4)
    ax.fill_between(range(len(y), len(y)+steps), p_ci[:, 0], p_ci[:, 1],
                    alpha=0.12, label='Intervalo de confianza 95%')
    for a, mu in zip(range(len(y), len(y)+steps), p_mu):
        ax.annotate(f"{mu:.1f}", (a, mu), textcoords='offset points',
                    xytext=(0, 10), ha='center', fontsize=9, fontweight='bold')
    ax.set_title(f'Proyección {variable_name}\nModelo {mejor["nombre"]}', fontsize=13, fontweight='bold')
...
def parse_args():
    parser = argparse.ArgumentParser(description="ARIMA universal interactivo")
    parser.add_argument("--file", type=str, default="", help="Ruta al CSV (opcional)")
    parser.add_argument("--variable", type=str, default="", help="Variable (opcional; se usa solo en algunos casos)")
...
def main():
    ...
    try:
        ...
        steps = args.steps if args.steps and args.steps > 0 else ask_int("¿Cuántos pasos quieres pronosticar?", default=3, min_value=1)
